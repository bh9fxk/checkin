...
# @Author CHERWIN✨✨✨
# -------------------------------
# cron "15 0,6 * * *" script-path=xxx.py,tag=匹配cron用
# const $ = new Env('蜜雪冰城')
import json
import base64
import os
...
''')
    ...
    if IS_DEV:
        ...
        if os.path.isfile('CHERWIN_TOOLS.py'):
            ...
            if down_file('CHERWIN_TOOLS.py', 'https://github.com/bh9fxk/checkin/raw/main/CHERWIN_TOOLS.py'):
                print('脚本依赖下载完成请重新运行脚本')
                import_Tools()
            else:
                print('脚本依赖下载失败，请到https://github.com/bh9fxk/checkin/raw/main/CHERWIN_TOOLS.py下载最新版本依赖')
                exit()
    print(TIPS)
    token = ''
